...
$modeMessage = if ($vmrunUsable) { "vm-run mode: vmrun_guest_ops" } else { "vm-run mode: ssh_primary" }
Write-Output $modeMessage
Write-Output ("vm-run probe: " + $probeMode)
...
if ($NoWait) {
    Start-Process -FilePath $config.SshExe -ArgumentList $sshArgs | Out-Null
    exit 0
}
