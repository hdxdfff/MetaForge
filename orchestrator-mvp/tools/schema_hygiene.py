...
CORE_MESSAGES_FALLBACK_PATH = DATA / "core_messages.runtime.json"
...
def _write_json(path: Path, payload: Any) -> None:
    ...
    try:
        ...
        if path == CORE_MESSAGES_PATH:
            CORE_MESSAGES_FALLBACK_PATH.parent.mkdir(parents=True, exist_ok=True)
            CORE_MESSAGES_FALLBACK_PATH.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
            return
        raise
