...
DEFAULT_INDUSTRIAL_OPERATIONS_EVERY = 20
...
from tools.industrial_operations import run_industrial_operations
...
_APP_RUNTIME: tuple[Any, Any, Any] | None = None
_CONTROL_LAYER_RUNTIME: Any | None = None
_VERIFICATION_RUNTIME: Any | None = None
_IDENTITY_RUNTIME: tuple[Any, Any] | None = None
_AI_TESTING_RUNTIME: tuple[Any, Any] | None = None
_SELF_REPORT_RUNTIME: tuple[Any, Any, Any] | None = None
_OPENAI_RUNTIME: tuple[Any, Any, Any] | None = None


def _load_app_runtime() -> tuple[Any, Any, Any]:
    global _APP_RUNTIME
    if _APP_RUNTIME is None:
        from fastapi.testclient import TestClient
        from app.main import app, orchestrator

        _APP_RUNTIME = (app, orchestrator, TestClient(app))
    return _APP_RUNTIME


def _load_control_layer_runtime() -> Any:
    global _CONTROL_LAYER_RUNTIME
    if _CONTROL_LAYER_RUNTIME is None:
        from tools.meta_factory_control import run_control_layer

        _CONTROL_LAYER_RUNTIME = run_control_layer
    return _CONTROL_LAYER_RUNTIME


def _load_verification_runtime() -> Any:
    global _VERIFICATION_RUNTIME
    if _VERIFICATION_RUNTIME is None:
        from tools.verification_engine import run_verification

        _VERIFICATION_RUNTIME = run_verification
    return _VERIFICATION_RUNTIME


def _load_identity_runtime() -> tuple[Any, Any]:
    ...
    if _IDENTITY_RUNTIME is None:
        ...
        _IDENTITY_RUNTIME = (identity_kernel_status, refresh_identity_kernel)
    return _IDENTITY_RUNTIME


def _load_ai_testing_runtime() -> tuple[Any, Any]:
    global _AI_TESTING_RUNTIME
    if _AI_TESTING_RUNTIME is None:
        from tools.ai_testing_compat import run_ai_test_suite, load_ai_test_status

        _AI_TESTING_RUNTIME = (run_ai_test_suite, load_ai_test_status)
    return _AI_TESTING_RUNTIME


def _load_self_report_runtime() -> tuple[Any, Any, Any]:
    global _SELF_REPORT_RUNTIME
    if _SELF_REPORT_RUNTIME is None:
        from tools.factory_self_report import run_factory_self_report, send_factory_notification, should_emit_self_report

        _SELF_REPORT_RUNTIME = (run_factory_self_report, send_factory_notification, should_emit_self_report)
    return _SELF_REPORT_RUNTIME


def _load_openai_runtime() -> tuple[Any, Any, Any]:
    global _OPENAI_RUNTIME
    if _OPENAI_RUNTIME is None:
        ...
        try:
            ...
            OpenAI = None
        _OPENAI_RUNTIME = (settings, strategic_llm_api_style, OpenAI)
    return _OPENAI_RUNTIME

def _ensure_execution_recovery_direct() -> dict:
    _app, orchestrator, _client = _load_app_runtime()
...
def _render_human_friendly_metaforge_summary(*, milestone_hours: int, runtime: dict) -> dict[str, Any] | None:
    ...
    settings, strategic_llm_api_style, OpenAI = _load_openai_runtime()
    if OpenAI is None:
...
def _maybe_emit_metaforge_soak_milestone_notification(*, cycle: int, daemon_started_at: str | None) -> dict:
    ...
    for milestone_hours in METAFORGE_SOAK_MILESTONE_HOURS:
        ...
        _run_factory_self_report, send_factory_notification, _should_emit_self_report = _load_self_report_runtime()
...
def _run_ai_testing_summary() -> dict[str, object]:
    run_ai_test_suite, load_ai_test_status = _load_ai_testing_runtime()
    try:
...
def _collect_ai_testing_summary(
    ...
    _run_ai_test_suite, load_ai_test_status = _load_ai_testing_runtime()
    if future is None or not future.done():
        return None, future
    try:
...
def _status_cache_payload(result: dict | None, *, cycle: int, interval: int, meta_every: int, evolution_every: int, policy_schedule: dict | None = None, last_error: str | None = None) -> dict:
    ...
    industrial_operations = result.get('industrial_operations') or {}
    ...
    return {
        ...
        'industrial_operations': {
            'status': industrial_operations.get('status'),
            'freeze_triggered': ((industrial_operations.get('release_freeze') or {}).get('freeze_triggered')),
            'daily_actions_met': all(
                bool((item or {}).get('met'))
                for item in (industrial_operations.get('daily_actions') or {}).values()
            ) if industrial_operations.get('daily_actions') else None,
        },
        'self_model': {
...
def _compact_last_result(result: dict | None) -> dict:
    ...
    identity_kernel_status, _refresh_identity_kernel = _load_identity_runtime()
...
def daemon_status() -> dict:
    ...
    try:
        identity_kernel_status, _refresh_identity_kernel = _load_identity_runtime()
        identity_snapshot = identity_kernel_status(refresh=False) or {}
        identity_runtime = identity_snapshot.get('runtime') if isinstance(identity_snapshot, dict) else {}
        semantic_running = bool((identity_runtime or {}).get('daemon_running'))
    except Exception:
        semantic_running = False
    if not semantic_running:
        runtime_block = ((state.get('identity_gate') or {}).get('runtime') or {})
        semantic_running = bool(runtime_block.get('daemon_running'))
    state['pid'] = pid
    live_running = (running or semantic_running) and not stale
    state['running'] = live_running
    if live_running:
    ...
    if isinstance(last_result, dict):
        daemon_block = last_result.setdefault('daemon', {})
        if isinstance(daemon_block, dict):
            daemon_block['status'] = resolved_status
            daemon_block['running'] = live_running
            daemon_block['freshness_seconds'] = int(state.get('freshness_seconds') or 0)
            daemon_block['pid'] = pid
            daemon_block['last_tick_at'] = state.get('last_tick_at')
        runtime_block = (last_result.get('identity_gate') or {}).get('runtime')
        if isinstance(runtime_block, dict):
            runtime_block['daemon_status'] = resolved_status
            runtime_block['daemon_running'] = live_running
            runtime_block['daemon_fresh'] = live_running
        signal_dashboard = last_result.get('signal_dashboard')
        if isinstance(signal_dashboard, dict):
            runtime_panel = signal_dashboard.get('Runtime')
            if isinstance(runtime_panel, dict):
                runtime_panel['status'] = 'healthy' if live_running else 'attention'
                runtime_panel['signal_count'] = 0 if live_running else 1
...
def _policy_schedule(policy_state: dict, base_meta_every: int, base_evolution_every: int) -> dict:
    ...
    return {
        ...
        'run_coordination': False,
        'run_release_train': False,
        'inline_governance': bool(cadence.get('inline_governance', True)),
    }
...
def tick(run_meta: bool, run_evolution: bool, policy_schedule: dict | None = None) -> dict:
    ...
    try:
        ...
        industrial_operations_every = int((policy_schedule or {}).get('industrial_operations_every') or DEFAULT_INDUSTRIAL_OPERATIONS_EVERY)
        industrial_operations_due = industrial_operations_every <= 1 or cycle % industrial_operations_every == 0
        ...
        tick_trace(
            ...
            industrial_operations_due=industrial_operations_due,
        ...
        if recovery_payload.get('recovery_mode'):
            ...
            result = {
                ...
                'industrial_operations': {'status': 'signal-only', 'reason': 'execution_recovery_mode'},
        ...
        if run_meta and governance_enabled and guard.get('allow_meta', True):
            ...
            _app, _orchestrator, client = _load_app_runtime()
        ...
        if (run_meta or run_evolution) and observer_enabled and engineering_os_due:
            ...
            verification_runtime = _load_verification_runtime()
            verification_snapshot = verification_runtime(refresh_control_layer=False)
        ...
        if execution_priority_mode:
            ...
            control_layer_runtime = _load_control_layer_runtime()
            result['control_layer'] = control_layer_runtime(verification_snapshot=verification)
        ...
        if execution_priority_mode:
            result['industrial_operations'] = {'status': 'signal-only', 'reason': 'execution_priority_mode'}
            tick_trace('tick.industrial_operations_signal_only', cycle=cycle)
        elif not industrial_operations_due:
            result['industrial_operations'] = (
                {
                    **(previous_result.get('industrial_operations') or {}),
                    'cached': True,
                    'reason': 'governance cadence hold',
                }
                if previous_result.get('industrial_operations')
                else {'status': 'signal-only', 'reason': 'governance cadence hold'}
            )
            tick_trace('tick.industrial_operations_cached', cycle=cycle, cadence_every=industrial_operations_every)
        elif observer_enabled or governance_enabled:
            tick_trace('tick.before_industrial_operations', cycle=cycle)
            result['industrial_operations'] = run_industrial_operations()
            tick_trace('tick.after_industrial_operations', cycle=cycle, status=(result.get('industrial_operations') or {}).get('status'))
        else:
            result['industrial_operations'] = {'status': 'skipped', 'reason': 'lean_execution'}
            tick_trace('tick.industrial_operations_skipped', cycle=cycle)
...
def run_forever(interval: float, meta_every: int, evolution_every: int) -> int:
    ...
    _run_ai_test_suite, load_ai_test_status = _load_ai_testing_runtime()
    initial_ai_testing_status = load_ai_test_status()
    last_self_model_summary = _self_model_summary_from_state(_read_status_file())
    last_ai_testing_status = {
        **initial_ai_testing_status,
        'release_signal': (initial_ai_testing_status.get('release_signal') or 'sampled_async'),
        'mode': (initial_ai_testing_status.get('mode') or 'sampled_async'),
        'signal_only': True,
        'run_pending': False,
    }
        ...
        while True:
            ...
            try:
                ...
                if should_publish_runtime:
                    ...
                    _identity_kernel_status, refresh_identity_kernel = _load_identity_runtime()
                ...
                if not execution_priority_mode:
                    run_factory_self_report, _send_factory_notification, should_emit_self_report = _load_self_report_runtime()
