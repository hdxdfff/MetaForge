...
def _build_command(command: str, args: list[str]) -> list[str]:
    ...
    if command == "github-smoke":
        return [uv, "run", "python", "tools/github_api_smoke.py", *args]
    raise SystemExit(
        "Usage: python tools/python_tooling.py [sync|qa|ruff-check|ruff-format|ty-check|github-smoke] [args...]"
    )
...
def main(argv: list[str]) -> int:
    if len(argv) < 2:
        raise SystemExit(
            ...
            "[sync|qa|ruff-check|ruff-format|ty-check|github-smoke] [args...]"
        )
