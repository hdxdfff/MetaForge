...
def _path_writable(path: Path) -> bool:
    try:
        ...
        probe = path / f".graph_write_test_{uuid4().hex}.tmp"
...
def _graph_output_path(graph_id: str) -> Path:
    primary = GRAPHS / f"{graph_id}.json"
    if _path_writable(GRAPHS):
        return primary
    fallback = GRAPHS_FALLBACK / f"{graph_id}.json"
    GRAPHS_FALLBACK.mkdir(parents=True, exist_ok=True)
    return fallback
...
def compile_goal(goal: dict) -> dict:
    ...
    path = _graph_output_path(graph_id)
