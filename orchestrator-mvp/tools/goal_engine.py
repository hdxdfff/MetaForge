...
GOALS_FALLBACK = DATA / "goal_registry.json"
...
class GoalEngine:
    def __init__(self, path: Path = GOALS) -> None:
        self.path = self._resolve_goal_path(path)
        self._cached_signature: tuple[int, int] | None = None
        self._cached_goals: list[dict[str, Any]] | None = None

    def _resolve_goal_path(self, path: Path) -> Path:
        candidate = path
        if candidate != GOALS:
            return candidate
        if self._path_writable(candidate):
            return candidate
        if self._path_writable(GOALS_FALLBACK.parent):
    ...
    def _path_writable(self, path: Path) -> bool:
        try:
            ...
            probe = target / f".goal_registry_write_test_{uuid4().hex}.tmp"
    ...
    def _load_goals_cached(self) -> list[dict[str, Any]]:
        ...
        if changed:
            try:
                ...
                if self.path == GOALS and self._path_writable(GOALS_FALLBACK.parent):
                    self.path = GOALS_FALLBACK
                    self.save_goals(goals)
                else:
                    pass
    ...
    def save_goals(self, goals: list[dict[str, Any]]) -> None:
        ...
        try:
            ...
            if self.path != GOALS_FALLBACK and self._path_writable(GOALS_FALLBACK.parent):
                self.path = GOALS_FALLBACK
                atomic_write_json(self.path, merged_goals)
            else:
                raise
