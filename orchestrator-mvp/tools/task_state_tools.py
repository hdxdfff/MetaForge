...
RUNTIME_TASKS_FALLBACK = DATA / "runtime" / "tasks"
GRAPHS = FACTORY / "graphs"
GRAPHS_FALLBACK = DATA / "graphs"
...
def _save_json(path: Path, payload: Any) -> Path:
    try:
        ...
        if GRAPHS in path.parents or path.parent == GRAPHS:
            fallback = GRAPHS_FALLBACK / path.name
            fallback.parent.mkdir(parents=True, exist_ok=True)
            atomic_write_json(fallback, payload)
            return fallback
        if RUNTIME_TASKS in path.parents or path.parent == RUNTIME_TASKS:
            fallback = RUNTIME_TASKS_FALLBACK / path.relative_to(RUNTIME_TASKS)
            fallback.parent.mkdir(parents=True, exist_ok=True)
            atomic_write_json(fallback, payload)
            return fallback
        raise
...
def reconcile_execution_evidence_steps() -> dict[str, Any]:
    ...
    for task in tasks:
        ...
        written_path = _save_json(path, payload)
        result = task.setdefault("result", {})
        result["execution_evidence_path"] = str(written_path)
