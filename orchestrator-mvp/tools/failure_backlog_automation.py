...
def _template_payload(template: dict[str, Any], *, repo_path: str, cluster: dict[str, Any], dispatch_now: bool) -> dict[str, Any]:
    ...
    if queue_name == "exploration" or task_type in P3_TASK_TYPES:
        scheduler_hint["budget_pool"] = "P3"
    elif task_type in P2_TASK_TYPES or str(template.get("category") or "").strip().lower() in {"artifact_growth", "capability_expansion"}:
        scheduler_hint["budget_pool"] = "P2"
    else:
        scheduler_hint["budget_pool"] = "P1"
...
async def _create_tasks(payloads: list[dict[str, Any]]) -> list[dict[str, Any]]:
    try:
        ...
        for payload in payloads:
            ...
            scheduler_hint = dict(payload.get("scheduler_hint") or {})
            task_status = "planning" if bool(scheduler_hint.get("dispatch_now")) else "queued"
            task = {
                "id": task_id,
                ...
                "scheduled_by": payload.get("scheduled_by") or "runtime.failure_backlog_automation",
                "auto_approve": bool(payload.get("auto_approve", False)),
                "context_mode": payload.get("context_mode") or "lean",
                "allow_resource_scan": bool(payload.get("allow_resource_scan", True)),
                "allow_repo_status": bool(payload.get("allow_repo_status", True)),
                "preferred_worker": payload.get("preferred_worker"),
                "scheduler_hint": scheduler_hint,
                "context_package": payload.get("context_package") or {},
                "tool_route": payload.get("tool_route") or {},
                "executor_hint": payload.get("executor_hint") or {},
                "execution_mode": payload.get("execution_mode") or "governance",
                "execution_lane": payload.get("execution_lane") or "host-control",
                "worker_vm_policy": payload.get("worker_vm_policy") or {},
                "prompt_guard": payload.get("prompt_guard") or {},
                "tool_policy": payload.get("tool_policy") or {},
                "security_review": payload.get("security_review") or {},
                "artifact_spec": payload.get("artifact_spec") or {},
                "verification_contract": payload.get("verification_contract") or {},
                ...
                "rollback_rule": payload.get("rollback_rule"),
                "status": task_status,
                "created_at": now,
                "updated_at": now,
                "result": {},
            ...
            created.append(
                {
                    ...
                    "executor_id": None,
                    "template_id": scheduler_hint.get("template_id"),
                    "cluster_key": scheduler_hint.get("cluster_key"),
                    "handling_mode": scheduler_hint.get("handling_mode"),
                    "automation_source": scheduler_hint.get("automation_source"),
                    "creation_mode": "fallback-direct",
                    "creation_error": fallback_reason,
                }
            )
...
def run_failure_backlog_automation(mode: str = "full") -> dict[str, Any]:
    ...
    goal_health = str(goal_backlog_status.get("goal_health") or "").strip().lower()
    replenishment_result = str(goal_backlog_status.get("last_replenishment_result") or "").strip().lower()
    replenishment_block_reason = str(goal_backlog_status.get("replenishment_block_reason") or "").strip().lower()
    emergency_replenishment = (
        goal_health == "starved"
        or replenishment_result == "replenishment_failed"
        or replenishment_block_reason.startswith("dispatch_failed")
        or "dispatch_precheck_failed" in replenishment_block_reason
    )

    toyos_plan: dict[str, Any] | None = None
    if toyos_present and (mode != "light" or emergency_replenishment):
        ...
        toyos_report = {
            "status": "planned" if mode != "light" else "emergency",
