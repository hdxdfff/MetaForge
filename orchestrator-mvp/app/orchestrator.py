...
INDUSTRIAL_OPERATIONS_PATH = CODEX_ROOT / "orchestrator-mvp" / "data" / "industrial_operations.json"
...
TASK_POOL_VALUES = {"P0", "P1", "P2", "P3"}
TASK_POOL_P2_TASK_TYPES = {
    "new_artifact_bootstrap",
    "new_demo_creation",
    "architecture_migration",
    "subsystem_refactor",
...
TASK_POOL_P3_TASK_TYPES = {"exploration_intake"}
P2_SANDBOX_TASK_TYPES = {
    "doc_patch",
    "report_refresh",
    "json_fix",
    "manifest_patch",
...
TASK_CHECKPOINT_RESTORE_STATUSES = TASK_FINAL_STATUSES | {
...
def _checkpoint_terminal_status(checkpoint: dict[str, Any]) -> TaskStatus | None:
    ...
    try:
        ...
        return None
    if status in TASK_CHECKPOINT_RESTORE_STATUSES:
        return status
    return None
...
class Orchestrator:
    def __init__(self) -> None:
        ...
        self._current_dispatch_context: dict[str, Any] = {}
        self._load_state()
    ...
    async def create_task(self, payload: TaskCreate) -> TaskRecord:
        ...
        async with self._lock:
            with self._persist_lock:
                ...
                self._persist()
        enqueue_only = str((task.scheduler_hint or {}).get("dispatch_boundary") or "").strip().lower() == "enqueue" or bool((task.scheduler_hint or {}).get("enqueue_only"))
        if task.status != TaskStatus.waiting_approval and not enqueue_only:
            self._executor_submit(task)
        if not enqueue_only and guard_decision != "block" and not deferred_backlog and task.status != TaskStatus.waiting_approval:
    ...
    async def dispatch_task(self, payload: DispatchTaskRequest) -> TaskRecord:
        ...
        enqueue_only = bool(payload.enqueue_only or str(scheduler_hint.get("dispatch_boundary") or "").strip().lower() == "enqueue")
        ...
        if enqueue_only:
            ...
            surface_route = {
                ...
                "launch_mode": "enqueue",
                "launch_hint": "dispatch enters queue; execution claims independently",
            }
            vm_template = None
            worker_vm_policy = {
            ...
            context_package = {
                "dispatch_boundary": "enqueue",
                "dispatch_boundary_version": 1,
                "workspace": repo_path,
                "goal": payload.goal,
            }
            strategy_context = {}
            effective_preferred_worker = payload.preferred_worker
            scheduler_hint["dispatch_boundary"] = "enqueue"
            scheduler_hint["dispatch_boundary_mode"] = "enqueue"
            scheduler_hint["admission_mode"] = "degraded"
            scheduler_hint.setdefault("queue_name", "fastlane")
            scheduler_hint.setdefault("task_type", "doc_patch")
            scheduler_hint.setdefault("verification_level", "L1")
            scheduler_hint.setdefault("max_runtime_seconds", 900)
            scheduler_hint.setdefault("retry_limit", 0)
            scheduler_hint.setdefault("rollback_rule", "mark_failed_and_requeue_split")
        else:
        ...
        budget_pool = self._infer_budget_pool(
            payload=payload,
            execution_mode=execution_mode,
            task_type=task_type,
            queue_name=queue_name,
        )
        sandbox_lane = str(scheduler_hint.get("sandbox_lane") or "").strip().lower()
        budget_gate = self._enforce_budget_pool_admission(
        ...
        if not enqueue_only:
            ...
            throughput_gate["admission_mode"] = "enqueue_only"
        ...
        scheduler_hint["budget_pool"] = budget_pool
        scheduler_hint["budget_gate"] = budget_gate
        scheduler_hint["throughput"] = throughput_gate
        tool_policy = build_task_tool_policy(
            preferred_worker=effective_preferred_worker,
            tool_route=tool_route,
            repo_path=repo_path,
        ...
        if task.status != TaskStatus.waiting_approval:
            self._record_amplifier_event(task, entered_execution=1)
        if project:
    ...
    def _build_first_recovery_plan(self, task: TaskRecord, reason: str) -> PlannerResponse:
        ...
        if any(token in corpus for token in ("toyos", "toy os", "qemu", "kernel")):
            target_path = CODEX_ROOT / "generated" / "toy-os-demo"
...
""",
    ...
    async def _run_task(self, task_id: str) -> None:
        ...
        try:
            ...
            if verification_result.get("passed"):
                self._record_amplifier_event(task, completed_tasks=1)
            self._persist()
    ...
    def _industrial_operations_snapshot(self) -> dict[str, Any]:
        try:
            raw = json.loads(INDUSTRIAL_OPERATIONS_PATH.read_text(encoding="utf-8-sig"))
            return raw if isinstance(raw, dict) else {}
        except Exception:
            return {}

    def _infer_budget_pool(
        self,
        *,
        payload: DispatchTaskRequest,
        execution_mode: ExecutionMode,
        ...
        explicit = str(
            (payload.scheduler_hint or {}).get("budget_pool")
            or (payload.scheduler_hint or {}).get("task_pool")
            or payload.admission_lane
            or ""
        ).strip().upper()
        if explicit in TASK_POOL_VALUES:
            return explicit
        if queue_name == "exploration" or task_type in TASK_POOL_P3_TASK_TYPES or execution_mode == ExecutionMode.research:
            return "P3"
        if task_type in TASK_POOL_P2_TASK_TYPES:
            return "P2"
        return "P1"

    def _enforce_budget_pool_admission(self, *, budget_pool: str, task_type: str = "", sandbox_lane: str = "") -> dict[str, Any]:
        snapshot = self._industrial_operations_snapshot()
        budget_control = snapshot.get("budget_control") if isinstance(snapshot, dict) else {}
        gates = budget_control.get("gates") if isinstance(budget_control, dict) else {}
        usage = budget_control.get("usage_24h") if isinstance(budget_control, dict) else {}
        sandbox_lane = str(sandbox_lane or "").strip().lower()
        task_type = str(task_type or "").strip().lower()
        if budget_pool == "P2":
            if sandbox_lane == "p2_low_risk":
                if not bool(gates.get("allow_p2_sandbox", False)):
                    raise ValueError("Throughput admission rejected: industrial budget gate closed for P2 sandbox tasks.")
                if task_type not in P2_SANDBOX_TASK_TYPES:
                    raise ValueError("Throughput admission rejected: task_type is not eligible for the P2 sandbox lane.")
                if int((usage.get("sandbox_p2_daily") or {}).get("remaining_24h") or 0) <= 0:
                    raise ValueError("Throughput admission rejected: P2 sandbox budget exhausted.")
                return {
                    "industrial_operations_status": snapshot.get("status"),
                    "budget_pool": budget_pool,
                    "sandbox_lane": sandbox_lane,
                    "allow_p2": bool(gates.get("allow_p2", False)),
                    "allow_p2_sandbox": bool(gates.get("allow_p2_sandbox", False)),
                    "allow_p3": bool(gates.get("allow_p3", False)),
                }
            if not bool(gates.get("allow_p2", False)):
                raise ValueError("Throughput admission rejected: industrial budget gate closed for P2 tasks.")
            if int((usage.get("capability_expansion_daily") or {}).get("remaining_24h") or 0) <= 0:
                raise ValueError("Throughput admission rejected: capability expansion budget exhausted.")
        if budget_pool == "P3":
            if not bool(gates.get("allow_p3", False)):
                raise ValueError("Throughput admission rejected: industrial budget gate closed for P3 tasks.")
            if int((usage.get("research_daily") or {}).get("remaining_24h") or 0) <= 0:
                raise ValueError("Throughput admission rejected: research budget exhausted.")
        return {
            "industrial_operations_status": snapshot.get("status"),
            "budget_pool": budget_pool,
            "allow_p2": bool(gates.get("allow_p2", False)),
    ...
    def _amplifier_name(self, task: TaskRecord) -> str | None:
        scheduler_hint = task.scheduler_hint or {}
        for key in ("amplifier", "amplifier_name", "source_amplifier"):
            value = scheduler_hint.get(key)
            if isinstance(value, str) and value.strip():
                return value.strip()
        goal_admission = task.goal_admission or {}
        value = goal_admission.get("amplifier")
        if isinstance(value, str) and value.strip():
            return value.strip()
        return None

    def _record_amplifier_event(
        self,
        task: TaskRecord,
        *,
        ...
        amplifier = self._amplifier_name(task)
        if not amplifier:
            return
        try:
            record_run(
                ...
                metadata={
                    "task_id": task.id,
                    "task_type": task.task_type,
                    "budget_pool": (task.scheduler_hint or {}).get("budget_pool"),
                    "sandbox_lane": (task.scheduler_hint or {}).get("sandbox_lane"),
                },
            )
        except Exception:
            return
