...
function renderOverview() {
  ...
  const totalSignals = Object.values(sig).reduce((sum, x) => sum + (x?.signal_count || 0), 0);
  ...
  if (insightStrip) {
    const toBars = (values) => {
      ...
      return nums.map((value, index) => {
        ...
        return `<span class="spark-bar ${index === nums.length - 1 ? "latest" : ""}" style="height:${height}px"></span>`;
      }).join("");
    };
    const releaseState = zh(s.release_operations?.release_train_status || s.release_operations?.status || "unknown");
    insightStrip.innerHTML = [
      {
        title: "队列轨道",
        value: queueDepth,
        copy: `${queueRunning} 进行中 · ${queueProblem} 问题`,
        bars: toBars([q.queued, q.running, q.done, q.problem]),
      },
      {
        title: "信号轨道",
        value: totalSignals,
        copy: `${zh(sig.Runtime?.status || "unknown")} / ${zh(sig.Maturity?.status || "unknown")} / ${zh(sig["Release Readiness"]?.status || "unknown")}`,
        bars: toBars([sig.Runtime?.signal_count || 0, sig.Maturity?.signal_count || 0, sig["Release Readiness"]?.signal_count || 0]),
      },
      {
        title: "发布轨道",
        value: releaseState,
        copy: `${artifactCount || freshCount} 个真实产物 · ${qualityGateLabel}`,
        bars: toBars([artifactCount, freshCount, blockerCount, queueDone]),
      },
    ].map((item) => `
      <article class="insight-card">
        <div class="insight-card-head">
          ...
          <span class="pill subtle-pill">${esc(item.copy)}</span>
        </div>
        <div class="sparkline" aria-hidden="true">${item.bars}</div>
      </article>`).join("");
  }
...
function renderTaskDetail() {
  ...
  const graph = task.graph && typeof task.graph === "object" ? task.graph : null;
  const graphNodes = Array.isArray(graph?.nodes) ? graph.nodes : [];
  const graphEdges = Array.isArray(graph?.edges) ? graph.edges : [];
  detail.innerHTML = `
    <div class="stack">
      <div class="detail-actions">
        <button type="button" class="ghost-btn" data-action="open-task-raw">打开原始数据</button>
        <button type="button" class="ghost-btn" data-action="open-task-graph">打开图谱</button>
        <button type="button" class="ghost-btn" data-action="open-task-evidence">打开证据</button>
      </div>
      ...
      ${graph ? `<article id="task-graph-card" class="event-card"><h3>任务图谱</h3><div class="detail-copy">节点 ${graphNodes.length} · 连边 ${graphEdges.length}</div><div class="graph-grid">${graphNodes.slice(0, 6).map((node) => `<div class="graph-node"><span>${esc(node.label || node.name || node.id || "node")}</span><strong>${esc(zh(node.kind || node.type || node.status || "node"))}</strong></div>`).join("")}</div></article>` : ""}
...
function bindEvents() {
  ...
  const handlers = {
    ...
    "open-task-raw": () => {
      switchView("evidence");
      setTimeout(() => document.getElementById("raw-task")?.scrollIntoView({ behavior: "smooth", block: "start" }), 80);
    },
    "open-task-graph": () => {
      switchView("mission");
      setTimeout(() => document.getElementById("task-graph-card")?.scrollIntoView({ behavior: "smooth", block: "start" }), 80);
    },
    "open-task-evidence": () => {
      switchView("evidence");
      setTimeout(() => document.getElementById("evidence")?.scrollIntoView({ behavior: "smooth", block: "start" }), 80);
    },
