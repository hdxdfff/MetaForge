...
def run_control_layer(verification_snapshot: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    quality = load_snapshot(QUALITY, {"status": "unknown", "overall_score": 0.0}, previous_payload.get("quality_system"))
    ...
    autonomy = load_snapshot(DATA / "autonomy_score.json", {}, previous_payload.get("autonomy_score"))
    ...
    latest_tool_health = tool_recent[-1] if tool_recent else previous_payload.get("tool_health") or {}
    tool_health = {
    ...
    ai_testing = load_snapshot(
        DATA / "ai_test_status.json",
        {"status": "disabled", "pass_rate": None, "error_count": 0, "failing_case_ids": []},
        previous_payload.get("ai_testing"),
    )
