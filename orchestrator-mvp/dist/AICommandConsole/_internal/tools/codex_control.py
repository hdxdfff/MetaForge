...
def cmd_rnd_pipeline_status(_args):
    out(build_rnd_delivery_pipeline_status())
...
def cmd_control_layer_status(args):
    payload = (
        run_control_layer()
        if args.refresh or not _control_layer_cache_is_fresh()
        else _read_control_layer_cache()
    )
